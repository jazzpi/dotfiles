...
if ret == 127 then
  naughty.notify({ text = "Please install laptop-detect\nAssuming a desktop PC" })
end
...
beautiful.init(awful.util.getdir("config") .. "themes/jazzpi/theme.lua")
naughty.config.defaults.icon_size = beautiful.naughty_icon_size or 48
...
mycpulabel.font = beautiful.icon_font
mycpulabel.text = "\u{f003}" -- icon-cpualt-processoralt
mycpuwidget = wibox.widget.textbox()
mycpuwidget.forced_width = 35
vicious.register(mycpuwidget, vicious.widgets.cpu, "$1%")

-- RAM widget
myramlabel = wibox.widget.textbox()
myramlabel.font = beautiful.icon_font
myramlabel.text = "\u{f02c}" -- icon-ram
...
-- Tag names using WebHostingHubGlyphs icons
local tag_names = {
  "\u{f01d}", -- icon-domain
  "\u{21b8}", -- icon-home
  "\u{f5c3}", -- icon-terminalalt
...
awful.rules.rules = {
    ...
    { rule = { class = "Chromium" },
      properties = { screen = 1, tag = tag_names[1] } },
    { rule = { class = "Emacs" },
